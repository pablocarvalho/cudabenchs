...
class ApplicationRunner(DBConnection):
    def run(self):
        ...
        self.cursor.execute("""SELECT * FROM Application AS App INNER JOIN Benchmark AS BM
        ...
        for row in self.cursor.fetchall():
            ...
            db_name = format_name(DEVICE_NAME) + "_" + format_name(row['name']) + "_" + format_name(row["acronym"]) + ".db"
        ...
        for num_iteration in range(num_iterations):
            ...
            for (db, app_id) in db_list:
                ...
                for nvprof_row in nvprof_rows:
                    self.cursor = self.connection.cursor()
                    self.cursor.execute("SELECT _id_ FROM Kernels WHERE mangledName = ?;",
                                        (nvprof_row['value'],))
